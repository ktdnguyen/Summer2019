...
(
    ...
    from country as a
	join countrylanguage as b on b.CountryCode=a.Code
) 
...
select 
	...
	from languageinfo as a
	where a.language not in (select language from top10language)and a.percentage > 50
    )
...
select count(*)  / (select count(*) from country) as fraction_of_countries_Speaking_less_popular_languages
from
(select * 
	from languageinfo as a
	where a.language not in (select language from top10language)and a.percentage > 50
...
b.district as DistrictName, b.districtpop as DistrictPopulation, 
b.cityCount as CityCount_District, b.countrycode as CountryCode,
